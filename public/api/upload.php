<?php
/**
 * 图片上传接口 —— 把 AI / 定时任务产出的配图上传到站点，返回可直接引用的 URL。
 *
 * 方法: POST   Content-Type: multipart/form-data
 * 鉴权: 同 /api/push.php —— 见 src/auth.php
 ...
 *
 * 表单字段:
 *   files[]  string  必填。图片文件，可多张（上限见 config.php upload.max_files）
 *   file     string  files[] 的等价单文件写法
 *   name     string  可选。文件名主干，如 10-person-team-ai-efficiency-01
 *                    非 ASCII 字符会被剔除；多张同时上传时自动追加 -01/-02 序号；
 *                    留空则用「日期-随机串」
 *   alt      string  可选。用于生成 <img alt="...">
 *
 * 成功响应(200):
 * {
 *   "success": true,
 ...
 *       "html":   "<img src=\"...\" alt=\"\" width=\"1200\" height=\"675\" loading=\"lazy\">",
 *       "markdown":"![](https://...jpg)",
 *       "size":   184320,
 *       "mime":   "image/jpeg",
 *       "width":  1200,
 ...
 * 与 AI 自动入库的配合：
 *   ① 本接口先上传配图 → 拿到 url
 *   ② 把 url 写进文章的 content(<img>) 与 cover_image
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    api_json(405, [
        ...
        'message' => '请使用 POST 上传图片（multipart/form-data）',
    ]);
}
...
if (empty($_FILES) && empty($_POST)) {
    ...
    if ($len > 0 && $postMax > 0 && $len > $postMax) {
        push_log($token['id'], $token['api_key'], 'upload', 'error',
        ...
        api_json(413, [
            ...
            'hint'    => '请调大 PHP 的 post_max_size / upload_max_filesize，或减少单次上传张数/单张体积',
        ]);
    }
}
...
if (!$files) {
    push_log($token['id'], $token['api_key'], 'upload', 'error', '未收到文件', 0);
    api_json(400, [
        'success' => false,
        'error'   => 'no_file',
...
if (count($files) > (int)$cfg['max_files']) {
    push_log($token['id'], $token['api_key'], 'upload', 'error', '文件数超限 ' . count($files), 0);
    api_json(400, [
        'success' => false,
        'error'   => 'too_many_files',
        'message' => '单次最多上传 ' . (int)$cfg['max_files'] . ' 张，本次收到 ' . count($files) . ' 张',
    ]);
}
...
foreach ($files as $i => $f) {
    try {
        // 多张共用同一 name 时，按现有约定自动追加 -01/-02 序号
        $hint = ($nameHint !== '' && $total > 1)
            ? rtrim($nameHint, '-') . '-' . str_pad((string)($i + 1), 2, '0', STR_PAD_LEFT)
            : $nameHint;

        $meta = upload_store_file($f, $hint);

        // 顺手给出可直接粘贴的片段，省去调用方拼接
        $meta['html']     = '<img src="' . attr($meta['url']) . '" alt="' . attr($alt) . '"'
            . ' width="' . (int)$meta['width'] . '" height="' . (int)$meta['height'] . '" loading="lazy">';
        $meta['markdown'] = '![' . str_replace([']', ')'], '', $alt) . '](' . $meta['url'] . ')';
...
if (!$stored) {
    ...
    push_log($token['id'], $token['api_key'], 'upload', 'error', '全部失败：' . $first, 0);
    api_json(400, [
        'success' => false,
        'error'   => 'upload_failed',
        'message' => $first,
...
push_log($token['id'], $token['api_key'], 'upload', $partial ? 'error' : 'success',
    '上传 ' . count($stored) . ' 张' . ($partial ? '，失败 ' . count($errors) . ' 张' : ''),
    count($stored));
